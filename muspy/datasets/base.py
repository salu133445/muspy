...
class MusicDataset:
    ...
    def download(self, subsets=None, extract=True, cleanup=False):
        ...
        for subset in subsets:
            if subset in self.sources:
                source = self.sources[subset]
            else:
                warnings.warn("Skipped unrecognized subset: {}.".format(subset))
            download_url(
                source["url"], self.root, source["filename"], source.get("md5")
            )
            if extract:
                extract_archive(
                    os.path.join(self.root, source["filename"]),
                    remove_finished=cleanup,
                )
